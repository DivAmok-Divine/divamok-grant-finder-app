...
export default function Intake() {
  ...
  const { setRun, grants } = useStore()

  const [audience, setAudience] = useState<RecipientType>('startup')
  const [company, setCompany] = useState('')
  const [country, setCountry] = useState('')
  const [stage, setStage] = useState('Idea')
  const [sectors, setSectors] = useState<string[]>(['Clean energy'])
  const [pitch, setPitch] = useState('')
  const [funding, setFunding] = useState('$100k – $500k')
  ...
  function run() {
    ...
    setTimeout(() => {
      setRun(profile, matchGrants(profile, grants))
      navigate('/matches')
    }, 450)
  }
